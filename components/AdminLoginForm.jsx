...
export default function AdminLoginForm() {
  ...
  async function unlock() {
    if (!passInput) return;
    setPending(true);
    const result = await loginAction(passInput);
    setPending(false);
    if (result.ok) {
      router.refresh();
    } else {
  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", padding: "64px 24px", textAlign: "center" }}>
      ...
      <h2 style={{ fontFamily: FONT_SANS, fontSize: 20, color: INK, margin: "16px 0 8px" }}>Admin access</h2>
      <p style={{ fontSize: 12.5, color: SLATE, marginBottom: 20, lineHeight: 1.6 }}>
        Sign in to see every report generated by this app.
      </p>
      <input
        type="password"
        value={passInput}
      ...
      <div>
        <Link href="/" style={{ background: "none", border: "none", color: SLATE, fontSize: 13 }}>
          ← Back
        </Link>
      </div>
    </div>
