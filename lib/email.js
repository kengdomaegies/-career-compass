...
// Called right after a report is created: notifies the owner only. The
// client's email is still collected (for the coach's own follow-up) but
// isn't auto-emailed a copy — use the "send this report" box for that.
export async function notifyReportGenerated({ clientName, reportUrl }) {
  return Promise.all([sendOne({ to: OWNER_EMAIL, clientName, reportUrl, purpose: "owner" })]);
}
